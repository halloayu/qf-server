...
layui.use(["element", "layer", "okUtils", "okTab", "okLayer", "okContextMenu", "okHoliday"], function () {
	...
	function noticeFun() {
        ...
        if(isNoticeClick == 1){
            isNoticeClick = 0; // 当公告被看过以后
            let obj = document.getElementById("notice_dot");
            let parent = obj.parentNode;
            parent.removeChild(obj); // 删除红点提醒
        }
    }
	/**
	 * 项目地址
	 */
	$(".layui-footer button.address").click(function () {
		window.open("https://github.com/yzeng3/qf-server")
	});

	/**
	 * 项目信息
	 */
	$("body").on("click", ".layui-footer button.information", function () {
		layer.tab({
			area: ["auto", "370px"],
			tab: [{
				title: "作者",
				content: "<h3 style='margin-left: 100px; margin-top: 30px;'>yu zeng</h3>"
			}, {
				title: "指导老师",
				content: "<h3 style='margin-left: 100px; margin-top: 30px;'>杨洋</h1>"
			}, {
				title: "感谢",
				content: "<p style='margin-left: 100px; margin-top: 30px;'>感谢ok-admin开源模版作者bobi</p>"
			}]
		});
	});
